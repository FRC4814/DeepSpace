...
public class DriveTrain extends Subsystem {
    ...
    public static final DashboardVariable<Boolean> driveStraightOn = new DashboardVariable<Boolean>("drive straight",
            false);
    ...
    public DriveTrain() {
        ...
        // together
        SpeedControllerGroup leftGroup = new SpeedControllerGroup(new PWMVictorSPX(RobotMap.LEFT_MOTORS[0]),
                new PWMVictorSPX(RobotMap.LEFT_MOTORS[1]), new PWMVictorSPX(RobotMap.LEFT_MOTORS[2]));
        SpeedControllerGroup rightGroup = new SpeedControllerGroup(new PWMVictorSPX(RobotMap.RIGHT_MOTORS[0]),
                new PWMVictorSPX(RobotMap.RIGHT_MOTORS[1]));
    ...
    public static void curvDrive() {
        ...
        if (slow) {
            ...
            if (driveStraightOn.get()) {
                Robot.driveTrain.m_myRobot.curvatureDrive(throttle / 3,
                        -Robot.m_oi.myController.getX(Hand.kRight) * drivestraightPercent(), isQuickTurn);
            } else {
                Robot.driveTrain.m_myRobot.curvatureDrive(throttle / 3, -Robot.m_oi.myController.getX(Hand.kRight),
                        isQuickTurn);
            }
        } else {
            // checks if drive straight is on or off
            if (driveStraightOn.get()) {
                Robot.driveTrain.m_myRobot.curvatureDrive(throttle,
                        -Robot.m_oi.myController.getX(Hand.kRight) * drivestraightPercent(), isQuickTurn);
            } else {
                Robot.driveTrain.m_myRobot.curvatureDrive(throttle, -Robot.m_oi.myController.getX(Hand.kRight),
                        isQuickTurn);
            }
        }
    }
